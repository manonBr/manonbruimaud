...
const sentence = "Dévellopeuse front-end (React)";
class TextScramble {
    constructor(element) {
        this.element = element
...
// PORTFOLIO FILTER
const filters = document.querySelectorAll(".portfolio__filter .btn")
let portfolioItems = document.querySelectorAll(".portfolio__item")
portfolioItems = [...portfolioItems].filter((element) => !0 !== element.classList.contains("portfolio__item--divider"))
            ...
            if("all" === filter) {
                ...
                portfolioItems.forEach((item) => {
                    ...
                    if(item.dataset.type === filter) {
                        item.classList.remove("hidden")
                        if(item.nextSibling.nextSibling.classList.contains("portfolio__item--divider")) {
                            item.nextSibling.nextSibling.classList.remove("hidden")
                        }
                    } else {
